...
if __name__ == '__main__':
    parser = ArgumentParser(
        description='Benchmark for C++ LUT implementation')
    parser.add_argument('--lut_size', type=int, default=100,
                        help='Number of entries in lookup table.')
    parser.add_argument('--width', type=int, default=1920,
                        help='Width of destination.')
    parser.add_argument('--height', type=int, default=1080,
                        help='Height of destination.')
    parser.add_argument('--benchmark_duration', type=int, default=3,
    ...
    table = Table(rng.integers(low=0, high=255, endpoint=True, size=(args.lut_size, 4), dtype='uint8'))

    vmin = 10
    vmax = 20

    shape = (args.height, args.width)
    source = rng.uniform(low=vmin*0.95,
                         high=vmax*1.05,
                         size=shape)
